...
import { 
  ...
  Archive, Edit2, LogOut, UserPlus, Trash2, Filter, Info, RotateCcw, Download, Briefcase, Monitor
} from 'lucide-react';
    ...
    <div className="flex flex-col h-screen bg-[#f1f5f9] text-slate-800 font-sans">
      ...
      <div className="flex-1 overflow-hidden flex flex-col relative">
        <main className="absolute inset-0 overflow-y-auto p-4 md:p-6">
          {activeTab === 'overview' && (
            <FleetOverview ships={ships} crew={crew} today={today} />
          )}
          {activeTab === 'dashboard' && (
            <Dashboard 
...
function FleetOverview({ ships, crew, today }) {
  return (
    <div className="space-y-4 h-full flex flex-col">
      <div className="flex justify-between items-end shrink-0">
        <h1 className="text-2xl font-bold text-slate-800">Fleet Overview (Upcoming Crew Changes)</h1>
      </div>
      
      {/* 2x5 formati icin xl:grid-cols-5 kullanilmistir. Ekran genisligine gore uyarlanir. */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 flex-1 overflow-y-auto pb-4">
        {ships.map(ship => {
          ...
          return (
            <div key={ship.id} className={`bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[400px] relative group hover:shadow-md transition-shadow`}>
               <div className={`absolute top-0 left-0 w-full h-1.5 bg-${ship.color}-500`}></div>
               
               {/* Gemi Basligi */}
               <div className="p-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center mt-1 shrink-0">
                 <div className="font-bold text-slate-800 truncate" title={ship.name}>{ship.name}</div>
               ...
               <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-slate-50/50">
                  {upcomingDates.length === 0 ? (
                    ...
                    upcomingDates.map(date => {
                       ...
                       return (
                         <div key={date} className="border border-blue-100 bg-white rounded-lg overflow-hidden shadow-sm">
                           <div className="bg-blue-50 text-blue-800 text-[11px] font-bold px-2 py-1.5 border-b border-blue-100 flex items-center gap-1">
                             <Calendar size={12}/> {new Date(date).toLocaleDateString('en-GB')}
                           </div>
                           <div className="p-2 space-y-2">
                              {/* Katilacaklar */}
                              {onsigners.length > 0 && (
                                <div>
                                  <div className="text-[9px] font-bold text-emerald-600 uppercase mb-0.5 tracking-wide">Joining (Onsigner)</div>
                                  {onsigners.map(c => (
                                    <div key={c.id} className="text-xs text-slate-700 flex justify-between border-b border-slate-50 last:border-0 pb-0.5">
                                      <span className="font-medium truncate pr-1">{c.name}</span>
                                      <span className="text-slate-500 shrink-0 text-[10px] bg-slate-100 px-1 rounded">{c.rank}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              ...
                              {/* Ayrilacaklar */}
                              {offsigners.length > 0 && (
                                <div className={onsigners.length > 0 ? "pt-2 border-t border-slate-100" : ""}>
                                  <div className="text-[9px] font-bold text-amber-600 uppercase mb-0.5 tracking-wide">Leaving (Offsigner)</div>
                                  {offsigners.map(c => (
                                    <div key={c.id} className="text-xs text-slate-700 flex justify-between border-b border-slate-50 last:border-0 pb-0.5">
                                      <span className="font-medium truncate pr-1">{c.name}</span>
                                      <span className="text-slate-500 shrink-0 text-[10px] bg-slate-100 px-1 rounded">{c.rank}</span>
  ...
  const allComps = useMemo(() => {
    ...
    (matrix || []).forEach(m => {
      if (Array.isArray(m.competencies)) {
        m.competencies.forEach(c => compSet.add(c));
      }
    });
...
function PromotionsRecruitment({ promotions, matrix, ranks, currentUser, onAdd, onUpdate, onDelete, onUpdateStep }) {
  ...
  const handleExport = () => {
    ...
    displayData.forEach(d => {
      const stepsArray = Array.isArray(d.steps) ? d.steps : [];
      let row = `${d.type},${d.name},${d.rank},${d.status},${calculateAvg(stepsArray)}`;
      csv += row + "\n";
    });
    ...
    <div className="space-y-4 h-full flex flex-col">
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-wrap gap-3 items-end shrink-0 justify-between">
         <div className="flex gap-3">
             ...
             <div>
               <label className="block text-xs text-slate-400 mb-0.5">Type</label>
               <select className="border border-slate-300 rounded text-sm p-1.5 focus:outline-none w-32 bg-white" value={fType} onChange={e=>setFType(e.target.value)}>
         ...
         <button onClick={handleExport} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-1.5 rounded-lg text-sm font-bold shadow-sm transition-colors mb-0.5"><Download size={14} className="inline mr-1" /> Export</button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex-1 flex flex-col">
        <div className="overflow-x-auto flex-1 p-2">
           <div className="space-y-3">
              {displayData.map(p => {
                const stepsArray = Array.isArray(p.steps) ? p.steps : [];
                const avg = calculateAvg(stepsArray);
                const isRejected = stepsArray.some(s => s.reject);
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-slate-50 rounded-xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 overflow-y-auto flex-1 bg-slate-100/50">
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                 {(Array.isArray(editModal.steps) ? editModal.steps : []).map((step, idx) => {
                   const isAdminOrCTO = currentUser.role === 'admin' || currentUser.jobTitle === 'CTO';
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center border border-red-200 rounded bg-red-50/50 mt-4 cursor-pointer">
                              <label className={`flex items-center gap-1.5 text-xs font-bold cursor-pointer w-full h-full justify-center p-1 rounded transition-colors ${step.reject ? 'text-white bg-red-500' : 'text-red-500 hover:bg-red-100'}`}>
                                <input type="checkbox" disabled={!canEdit} checked={step.reject || false} onChange={e=>{
                                   ...
                                   if(isRej) onUpdateStep(editModal.id, idx, 'score', ''); 
